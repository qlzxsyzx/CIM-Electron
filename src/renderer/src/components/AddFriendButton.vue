<template>
    <div class="tool-container" @click="handleOpenSearchFirend">
        <SvgIcon iconName="bg-add-friend" class="tool-icon" />
        <div class="tool-name">查找好友</div>
    </div>
...
import { searchByUsername, applyAddFriend } from '../api/friend'
...
const handleAddFriend = () => {
    const applyFriendDto = {
        toUserId: searchFriendDialog.result.userId,
        remark: searchFriendDialog.addFriendForm.remark,
        applyReason: searchFriendDialog.addFriendForm.reason
    }
    applyAddFriend(applyFriendDto).then(res =>{
        if (res.code === 200) {
            ElMessage.success('申请已发送');
            searchFriendDialog.addFriendDialogVisible = false;
        }
    }).catch(err => {
        ElMessage.error(err.message);
    })
